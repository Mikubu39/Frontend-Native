...
- [x] **Làm lại bộ dữ liệu bài học theo hướng Duolingo (2026-08-24).**
  - [x] Bỏ 2 chủ đề bảng chữ cái khỏi `build()` (đã có trang học chữ riêng đọc bảng `characters`).
  - [x] `data_topics_c.py`: 4 chủ đề mới (Mua sắm, Đi lại, Sở thích, Sức khoẻ) -> map 14 chủ đề / 100 bài / 1.592 câu.
  - [x] Nút loa CHỈ ở LISTEN_AND_SELECT, LISTEN_AND_ARRANGE, SPEAKING. Câu dịch và chọn hình bỏ hẳn audio (596/1.592 câu có tiếng, trước là 1.400/1.579).
  - [x] Gỡ `<AudioButton onPress={() => {}} />` vẽ vô điều kiện ở `vocab-question.tsx` — thủ phạm của loa câm trên 845 câu.
  - [x] Bỏ ảnh khỏi LISTEN_AND_SELECT (203 câu lộ đáp án).
  - [x] `distinct_image_distractors`: ép 4 thẻ của câu chọn hình phải 4 emoji khác nhau.
  - [x] `romaji.py` + `metadata_json.romaji` trên mọi đáp án/thẻ rời tiếng Nhật (2.560 đáp án, 0 thiếu).
  - [x] `metadata_json.glossary` mỗi câu + `JapaneseText`/`DualText` nhận `glossary` -> chạm giữ vào chữ là ra cách đọc + nghĩa.
  - [x] `SpeakingQuestionCard` có nút "Nghe câu mẫu" (trước đây có file mà không có nút).
  - [x] Test `src/components/quiz/__tests__/audio-and-romaji.test.tsx` (8 ca). `tsc --noEmit` 0 lỗi, `jest` 15 suite / 88 test xanh.
  - [x] Kiểm chứng qua backend thật: 6 bài trải khắp lộ trình, 0 vi phạm chính sách; 902 URL media trong DB đều có file; 60 URL trong payload đều trả 200.
...
- **Skill của Antigravity (`.agents/skills/`) KHÔNG tự chạy trên Claude Code**: Claude Code chỉ quét `.claude/skills/`. Ba skill nhỏ (`state-keeper`, `goal-tracker`, `behavior-guard`) vẫn có hiệu lực vì nội dung đã được chép vào `AGENTS.md` — file mà `CLAUDE.md` `@`-import. `impeccable` thì không, nên nối bằng junction sang `.claude/skills/impeccable` thay vì chép bản thứ ba (3.5MB × 148 file) vào repo; junction đã được `.gitignore`.
- **Luật "phải `tsc --noEmit` xanh mới kết thúc turn" chuyển từ văn xuôi sang Stop hook** (`.claude/hooks/tsc-gate.mjs`): prose phụ thuộc vào việc agent nhớ, hook thì không bỏ qua được. Hook chỉ chạy khi có `.ts/.tsx` dirty (turn hỏi-đáp không tốn 7s), và tự nhả sau 3 lần chặn liên tiếp để một lỗi type bế tắc không khoá được vòng lặp. Viết bằng Node chứ không phải `jq` vì máy này không có `jq`.
