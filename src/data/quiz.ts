...
export const LESSON_TIPS: Record<string, LessonTip> = {
  lp1: {
    ...
    explanation:
      "Hiragana là bảng chữ cái mềm cơ bản của tiếng Nhật. Hãy làm quen với các từ chào hỏi phổ biến như こんにちは (Xin chào) và ありがとう (Cám ơn).",
    examples: [
      {
        japanese: "こんにちは (Konnichiwa)",
        translation: "Xin chào (Dùng ban ngày)",
      },
  ...
  lp2: {
    ...
    explanation:
      "Viết tiếng Nhật cần tuân theo quy tắc: từ trái qua phải, từ trên xuống dưới. Tập viết đúng nét sẽ giúp chữ cái cân đối và dễ đọc.",
    examples: [
      {
        japanese: "あ (A - Nét ngang trước, nét cong dọc sau)",
        translation: 'Nguyên âm "A"',
      },
      {
        japanese: "い (I - Nét móc trái dài hơn nét phải)",
        translation: 'Nguyên âm "I"',
      },
    ],
  },
  lp3: {
    ...
    explanation:
      "Khi nghe tiếng Nhật, hãy chú ý các phụ âm kép và trường âm (âm kéo dài). Trường âm có thể làm thay đổi hoàn toàn ý nghĩa của từ.",
    examples: [
  ...
  lp4: {
    ...
    explanation:
      "Luyện nói to từng âm rõ ràng. Tránh phát âm theo kiểu tiếng Việt mà hãy chú ý ngữ điệu lên xuống tự nhiên của từ.",
    examples: [
      {
      ...
      {
        japanese: "よろしくおねがいします (Yoroshiku)",
        translation: "Rất mong nhận được sự giúp đỡ",
      },
    ],
  },
  lp5: {
    ...
    formula: "A は B です",
    explanation:
      "Thử sức với toàn bộ các câu hỏi đọc, viết, nghe, nói kết hợp ngữ pháp cốt lõi để chính thức mở khóa cấp độ mới.",
    examples: [
      { japanese: "わたしは学生です。", translation: "Tôi là học sinh." },
      {
        japanese: "これにほんごの本です。",
        translation: "Đây là sách tiếng Nhật.",
      },
    ],
  },
};
  ...
  lp4: [
    {
      ...
      imageUrl:
        "https://images.unsplash.com/photo-1521791136368-1a46827d0515?w=300&h=200&fit=crop",
      answers: [
...
export const MOCK_VOCAB_QUESTIONS = LESSON_QUESTIONS.lp5.filter(
  (q) => q.type === "vocab",
) as VocabQuestion[];
export const MOCK_KANA_QUESTIONS = LESSON_QUESTIONS.lp5.filter(
  (q) => q.type === "kana",
) as KanaQuestion[];
export const MOCK_PICTURE_QUESTIONS = LESSON_QUESTIONS.lp5.filter(
  (q) => q.type === "picture",
) as PictureQuestion[];
export const MOCK_KANJI_FILL_QUESTIONS = LESSON_QUESTIONS.lp5.filter(
  (q) => q.type === "kanji-fill",
) as KanjiFillQuestion[];
