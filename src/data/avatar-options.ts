...
export function buildAvatarUrl(config: Partial<AvatarConfig> = {}): string {
  ...
  if (hasFacialHair) {
    params.set(
      "facialHair",
      FACIAL_HAIR_TO_PARAM[safeConfig.facialHair] ?? FACIAL_HAIR_TO_PARAM.beard,
    );
...
export function parseAvatarUrl(url?: string): AvatarConfig {
  ...
  try {
    ...
    const accessory =
      top === "hat" ? "hat" : accessories === "round" ? "glasses" : "none";
    ...
    return {
      ...
      skin: params.get("skinColor")
        ? `#${params.get("skinColor")}`
        : DEFAULT_AVATAR_CONFIG.skin,
      hair: TOP_TO_HAIR[top] || DEFAULT_AVATAR_CONFIG.hair,
      facialHair,
      outfit: params.get("clothesColor")
        ? `#${params.get("clothesColor")}`
